...
/**
 * RecyclerView的自定义基类,可以通过重写“onBindViewHolder”,在里面写具体控件的监听事件
 * 一定要注意布局复用的问题
 *
 * Do not treat position as fixed; only use immediately and call holder.getAdapterPosition()
 *      to look it up later
 * RecyclerView will not call onBindViewHolder again when the position of the item changes in the data set
 *      unless the item itself is invalidated or the new position cannot be determined.
 * For this reason, you should only use the position parameter while acquiring the related data item
 *      inside this method, and should not keep a copy of it.
 ...
 *
 * Created by 丛 on 2017/10/10 0010.
 * @param <B> Bean类
 */
...
public class BaseRecyclerViewAdapter<B> extends RecyclerView.Adapter<BaseRecyclerViewAdapter.BindingViewHolder> {
    ...
    public void removeAll() {
        beanList.clear();
        notifyDataSetChanged();
//        MyHandler.runOnUi(new Runnable() {
//            @Override
//            public void run() {
//                notifyDataSetChanged();
//            }
